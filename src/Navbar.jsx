...
export default function Navbar() {
  ...
  const linkClasses = `text-md btn btn-ghost hover:text-navbar-teal-blue active:text-navbar-red ${
    isOnHomePage ? (scrolled ? "text-gray-800" : "text-white") : "text-gray-800"
  }`;
    ...
    <div className={navbarClasses}>
      <div className="container mx-auto px-4 flex items-center justify-between h-16">
        ...
        <div className="lg:hidden z-50" ref={hamburgerRef}>
          <Hamburger
            color={scrolled ? "#000" : isMenuOpen ? "#000" : "#fff"}
            size={20}
            toggled={isMenuOpen}
            toggle={toggleMenu}
          />
      ...
      >
        <div className="flex flex-col px-10 items-end text-gray-700 justify-start h-full space-y-8 mt-36">
          {/* Menu Links */}
          {[
            { label: t("navbar.links.3"), path: "/" },
            { label: t("navbar.links.0"), path: "/hakkimizda" },
            { label: t("navbar.links.1"), path: "/galeri" },
            { label: t("navbar.links.2"), path: "/iletisim" },
          ].map((menu, idx) => (
            <button
              key={idx}
              className={`text-2xl ${
          ...
          <div className="flex lg:hidden relative">
            ...
            {isLanguageDropdownOpen && (
              <div className="absolute top-full mt-[-1px] w-full text-black text-sm bg-white border-l border-r border-b border-gray-200 shadow-lg rounded-b-lg">
                {[
                  { code: "tr", flag: "/images/turkey.png" },
                  { code: "en", flag: "/images/uk.png" },
                  { code: "de", flag: "/images/de.png" },
                  { code: "ru", flag: "/images/ru.png" },
                ].map((lang, idx) => (
                  <div
                    key={idx}
                    className="hover:bg-gray-100 flex items-center cursor-pointer rounded-lg"
